...
export default class MyHtmlWebpackPlugin {
   ...
   private hate = async (mainFilePathName?: string) => {
      ...
      let globalEntryObject = this.options.entry["global"] as MyHtmlWebpackPluginEntryObjectFB | undefined
   ...
   private bundleHtml = async (entryObject: MyHtmlWebpackPluginEntryObjectFB, globalEntryObject?: MyHtmlWebpackPluginEntryObjectFB) => {
   ...
   private injectLinkTags = (bundleResults: __bundleResults, styles: MyHtmlWebpackPluginEntryObjectPathOptions[]) => {

      let tag = ""
      for (let i = 0; i < styles.length; i++) {
         const stylePathOptions = styles[i]
         const filename =  this.cssFileNameHashes.get(stylePathOptions.import) || stylePathOptions.filename
   ...
   private injectScriptTags = (bundleResults: __bundleResults, jschunks: MyHtmlWebpackPluginEntryObjectJsChunksOptions[]) => {
      ...
      for (let i = 0; i < jschunks.length; i++) {
         ...
         tagMap.set(jschunk.inject || "body", `<script ${Object.entries(jschunk.attributes || {}).map(a => `${a[0]}${typeof a[1] === "string" ? "="+"\""+a[1]+"\"" : ""}`).join(" ")} src="${filename}"></script>`.replace(/\s+/g, " "))
      }
   ...
   private bundleCss = async (stylePathOptions: MyHtmlWebpackPluginEntryObjectPathOptions) => {
